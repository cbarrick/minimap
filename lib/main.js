...
class Main {
  ...
  activate () {
    ...
    if (atom.inDevMode()) {
      ...
      this.subscriptionsOfCommands = atom.commands.add('atom-workspace', {
        'minimap:-clean-block-decorations': () => {
          createdMarkers.forEach(m => m.destroy())
          createdMarkers.length = 0
        },
        'minimap:measure-lines-offset': () => {
          const editor = atom.workspace.getActiveTextEditor()
          const view = atom.views.getView(editor)
          const start = new Date()

          console.profile('lineOffset')
          ...
          setTimeout(() => {
            ...
            for (let i = 0; i < lineCount; i++) {
              offsets.push(view.pixelPositionForScreenPosition([i, 0]).top)
            }

            console.profileEnd('lineOffset')
            console.log('line offsets', offsets, 'computed in', new Date() - start, 'ms')
          }, 100)
        },
        'minimap:add-block-decorations-to-editor': () => {
          const editor = atom.workspace.getActiveTextEditor()
          ...
          let c = 0
          for (let i = 0; i < 10; i++) {
            c += Math.round(Math.random() * 300)
            const start = c
            c += Math.round(Math.random() * 300)
            const end = c

            const startPos = editor.buffer.positionForCharacterIndex(start)
            const endPos = editor.buffer.positionForCharacterIndex(end)

            const marker = editor.markBufferRange([startPos, endPos], { persistent: false, invalidate: 'touch' })
            const props = {
              type: 'block',
              item: getElement()
            }
            ...
            editor.decorateMarker(marker, props)
          }
        }
      })
    }
