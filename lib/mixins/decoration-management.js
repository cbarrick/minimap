...
export default class DecorationManagement extends Mixin {
  ...
  decorationsForType (type) {
    return Object.keys(this.decorationsById)
      .map(i => this.decorationsById[i])
  ...
  decorateMarker (marker, decorationParams) {
    ...
    if (decorationParams.plugin == null) {
    ...
    if (type === 'overlay') {
      ...
      this.registerToMarkerEvents(marker, type)
    }
    ...
    if (type === 'overlay') {
      console.log(this.decorationsForType('overlay'))
    } else {
  ...
  /**
   * Registers to the passed-in marker events for the specified decoration type.
   *
   * @param  {Marker} marker the marker to listen to
   * @param  {string} type   the decoration type that will be associated
   *                         to the marker
   * @access private
   */
  registerToMarkerEvents (marker, type) {
    const {id} = marker
  ...
  /**
   * Registers to marker events for an overlay decoration
   *
   * @param  {Marker} marker the marker to listen to
   * @access private
   */
  registerToOverlayMarkerEvents (marker) {
    const {id} = marker
    ...
    if (this.decorationMarkerChangedSubscriptions[id] == null) {
      ...
      marker.onDidChange((event) => {
        ...
        // this.invalidateOverlayDecorationForScreenRowsCache()
  ...
  /**
   * Return the inferred plugin's name based on the invoker's invoker source
   * file.
   *
   * @return {string} the inferred plugin name or undefined
   * @access private
   */
